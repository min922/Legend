...
//////////Firestore 사용자 데이터 받기///////////////////////////////////////////////////////////
        ...
        var ing_id_list = mutableSetOf<String>() //ing_id

        //UserSelect컬렉션(사용자 냉장고 재료 데이터)
        var user_refrigerator = ArrayList<ing_id_name_date>() //사용자 냉장고 재료&유통기한 받기
        var user_refrigerator_ing = ArrayList<String>() //사용자 냉장고 재료 받기
        ...
        db_firestore.collection("UserSelect") //작업할 컬렉션
            ...
            .addOnSuccessListener { result_UserSelect -> //성공할 경우
                ...
                //Step1. ing_id_list : 중복되지 않은(=mutableSetOf) 'id'(ing_id) 받아오는 변수
                //UserSelect에 식재료 이름이랑 유통기한 받는 문서가 달라서 'id'로 묶어줘야함
                for (document_ref in result_UserSelect){ //가져온 문서들은 result_UserSelect에 들어감

                    if ("id" in document_ref){
                        val ing_id = document_ref.data["id"] as String
                        ing_id_list.add(ing_id)
                    }
                    println("ing_id_list : $ing_id_list")
                }
                //Step2. 식재료 이름 & 유통기한 받기
                for (document_ref_2 in result_UserSelect){
                    ing_id_list.forEach{//ing_id_list를 순회하며 it을 요소로 받음
                        if (document_ref_2.data["id"] == it){
                            Log.w("it ", it)
                            Log.w("id ", document_ref_2.data["id"].toString())
                            //ing_date : 유통기한 변수
                            //Cal_Date_len1 : '5'를 '05'로 받아서 날짜 형식에 맞게 해주는 함수
                            val ing_date = Cal_Date_len1(document_ref_2.data["year"].toString()) +
                                    Cal_Date_len1(document_ref_2.data["month"].toString()) +
                                    Cal_Date_len1(document_ref_2.data["day"].toString())
                            println("ing_date : $ing_date")

                            if (ing_date != "nullnullnull"){
                                Cal_Date(ing_date) //유통기한 계산 함수
                            }

                            //ing_list : 한 재료의 id, 이름, 유통기한을 ing_id_name_date클래스로 받아옴
                            //user_refrigerator : 사용자 냉장고 재료&유통기한
                            //user_refrigerator_ing : 사용자 냉장고 재료
                            val ing_list = ing_id_name_date(document_ref_2.data["id"].toString(),
                                document_ref_2.data["menuname"].toString(), ing_date)
                            user_refrigerator.add(ing_list)

                            if ("menuname" in document_ref_2){
                                user_refrigerator_ing.add(document_ref_2.data["menuname"].toString())
                            }
                        }
                    }
                }
                println("user_refrigerator_ing : ${user_refrigerator_ing}")
                //println("user_refrigerator : ${user_refrigerator[0].menuname}") user_refrigerator에서 인덱스, 클래스 변수 끌어올 수 있음
...
//////////SQLite 레시피 데이터//////////////////////////////////////////////////////////////////////////
        ...
        //1차 추천(식재료 50프로 이상 보유시 추천 & 유통기한 빠른 순으로 제공)
        val sql_1 = "SELECT info.RECIPE_NM_KO, info.COOKING_TIME\n" +
        ...
        val cursor = db_sqlite.rawQuery(sql_1,null)
        cursor.run{
            while(moveToNext()){
