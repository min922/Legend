...
//    val markerOptions_market = MarkerOptions()
    ...
    private fun moveMap(latitude: Double, longitude: Double, itemname: String) {
        ...
        Log.d("efjnqrf", distance_list.toString())
        ...
        for (i in dist_list.indices){
            ...
            checkEnd(dist_list[i], itemname, searchday, i, marketlatLng)
            address_list[i].text = geocoder.getFromLocation(for_addr[i].lati, for_addr[i].longi, 10)
        ...
        call.enqueue(object : retrofit2.Callback<DataClass.market> { //맨 끝 찾아서 맨 끝 값 callapi함수에 넣어주기
            ...
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    ...
                    callApi(1, end_row, marketname, itemname, searchday, idx, marketlatLng)
            ...
            call.enqueue(object : retrofit2.Callback<DataClass.market> {
                ...
                override fun onResponse(
                    ...
                    if (response.isSuccessful) {
                        ...
                        val markerOptions_market = MarkerOptions() //마커찍기
                        markerOptions_market.icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_ROSE))
                        markerOptions_market.position(marketlatLng)
                        markerOptions_market.title(marketname)
                        markerOptions_market.snippet(response.body()!!.ListNecessariesPricesService.row[0].A_PRICE + " 원")

                        googleMap?.addMarker(markerOptions_market)
//                        Log.d("된다", response.body().toString())
                    }
                }
                override fun onFailure(call: Call<DataClass.market>, t: Throwable) {
