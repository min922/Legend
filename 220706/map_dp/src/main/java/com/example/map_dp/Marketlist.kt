...
class Marketlist {
    ...
    data class lati_logi(var lati :Double =0.0, var longi :Double =0.0) //상위 5개 위, 경도 저장

    var tmp_fifth = arrayListOf<lati_logi>()

    fun marketdist(
        search_lati: Double,
        search_logi: Double
    ): Pair<ArrayList<String>, ArrayList<lati_logi>> {
        ...
        for (i in 0..4){
            ...
            tmp_fifth.add(lati_logi(market_loc_lati[i].lati, market_loc_lati[i].longi)) //제일 가까운 5개 시장 경, 위도
        }
