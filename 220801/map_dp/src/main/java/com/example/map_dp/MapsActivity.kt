...
class MapsActivity : AppCompatActivity(), ConnectionCallbacks,
    ...
    private fun moveMap(latitude: Double, longitude: Double, itemname: String) {
        ...
        for (i in dist_list.indices){
            ...
            checkEnd(dist_list[i], itemname, searchday, marketlatLng, addr, market_distance)
        }
    ...
    ) { //맨 끝 찾아서 맨 끝 값 callapi함수에 넣어주기
        val tmp_name = marketname.replace(" ", "") //시장 이름 띄어쓰기 없애기
        ...
        call.enqueue(object : retrofit2.Callback<DataClass.market> {
            ...
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    ...
                    callApi(1, end_row, marketname, itemname, searchday,
                        marketlatLng, addr, market_distance)
        ...
        call.enqueue(object : retrofit2.Callback<DataClass.market> {
            ...
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    ...
                    addr, market_distance.toString()) // 클릭이벤트를 위해 각 마커의 상세정보용 데이터를 태그에 저장
